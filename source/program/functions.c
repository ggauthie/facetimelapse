...
Image* imagesInter(Image * image1, Image * image2, int nombre_image){
    ...
    for(int i=1;i<=nombre_image;i++){// il faut que le numéro d image commence à 1
        ...
        a=calculateWeight(nombre_image,i)*image2->ptrPixel[k*(image1->width)+j].blue;
		    b=calculateWeight(nombre_image,i)*image2->ptrPixel[k*(image1->width)+j].green;
		    c=calculateWeight(nombre_image,i)*image2->ptrPixel[k*(image1->width)+j].red;
		    d=(1-calculateWeight(nombre_image,i))*image1->ptrPixel[k*(image2->width)+j].blue;
		    e=(1-calculateWeight(nombre_image,i))*image1->ptrPixel[k*(image2->width)+j].green;
		    f=(1-calculateWeight(nombre_image,i))*image1->ptrPixel[k*(image2->width)+j].red;
...
Image* imagesInter(Image * image1, Image * image2, int nombre_image){
    ...
    for (int m=0;m<nombre_image;m++){
        tab_images[m].width = image1->width;
        tab_images[m].height = image1->height;
        tab_images[m].ptrPixel= (Pixel*)malloc((image1->width) * (image1->height) * sizeof(Pixel));
    }
