...
Pixel getPixel(Image * image, int width, int height){
    return (image->ptrPixel[width + height*image->width]);
}
...
void setPixel(Image* image, int x, int y, Pixel pixel){
  ...
  //  }else{
	image->ptrPixel[x +y*image->width].blue=pixel.blue;
	image->ptrPixel[x +y*image->width].red=pixel.red;
	image->ptrPixel[x +y*image->width].green=pixel.green;
//}
}
